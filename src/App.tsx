...
export default function App() {
  ...
  const [material, setMaterial] = useState<'pet' | 'paper'>('pet');
  const [selectedThickness, setSelectedThickness] = useState<number>(20);
  const [paperDensity, setPaperDensity] = useState<string>("80");
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (state.paperDensity) setPaperDensity(state.paperDensity);
  ...
  useEffect(() => {
    if (!loading) {
      localStorage.setItem('pet_calc_cache', JSON.stringify({ 
        ...inputs, 
        material, 
  ...
  }, [inputs, material, selectedThickness, paperDensity, pricePriority, mainPriority, loading]);

  const record = useMemo(() => {
    if (material === 'pet') {
      return data.find(r => r.thickness === selectedThickness) || data[0];
    } else {
      const density = parseNum(paperDensity) || 80;
      return {
        thickness: 0,
        weightPerM2: density,
        m2PerKg: density > 0 ? (1000 / density) : 0,
      };
    }
  }, [data, selectedThickness, material, paperDensity]);
  ...
  }, [selectedThickness, material, paperDensity]);
    ...
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      <header className="h-16 bg-white/80 backdrop-blur-md border-b border-border-subtle flex items-center justify-between px-6 shrink-0 z-10 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-bold text-slate-900 m-0 tracking-tight">ООО ИПК · Калькулятор ПЭТ и Бумаги</h1>
        </div>
      </header>
      ...
      <main className="flex-grow flex flex-col lg:flex-row gap-6 p-6 max-w-[1440px] mx-auto w-full">
        <aside className="w-full lg:w-[350px] shrink-0">
          <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl p-6 shadow-2xl flex flex-col gap-6 lg:sticky lg:top-22">
            ...
            {/* Segmented control for material selection */}
            <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/50">
              <button 
                type="button"
                onClick={() => setMaterial('pet')}
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${material === 'pet' ? 'bg-white text-slate-900 shadow-sm border border-slate-200/10' : 'text-text-muted hover:text-slate-950'}`}
              >
              ...
              <button 
                ...
                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${material === 'paper' ? 'bg-white text-slate-900 shadow-sm border border-slate-200/10' : 'text-text-muted hover:text-slate-950'}`}
              >
            ...
            {material === 'pet' ? (
              ...
              <div className="flex flex-col gap-1.5 p-3 bg-slate-50 rounded-xl border border-slate-100">
                <label className="text-[11px] font-bold text-text-muted uppercase tracking-wider">Плотность бумаги (г/м²)</label>
                <div className="relative flex gap-2">
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      setPaperDensity(formatted);
                    }}
                    placeholder="80"
                    className="w-full h-11 px-3 bg-white border border-slate-200 rounded-lg text-lg font-bold focus:ring-2 focus:ring-brand/10 focus:border-brand outline-none"
                  />
                  <span className="flex items-center text-xs font-bold text-slate-500 px-3 bg-white border border-slate-200 rounded-lg whitespace-nowrap">г/м²</span>
                </div>
                {/* Presets */}
                <div className="grid grid-cols-4 gap-1 mt-1">
                  {["45", "60", "80", "120", "150", "200", "250", "300"].map((density) => (
                    <button
                      key={density}
                      type="button"
                      onClick={() => setPaperDensity(density)}
                      className={`py-1 text-[10px] font-bold rounded-md border transition-all cursor-pointer ${paperDensity === density ? 'bg-amber-50 text-amber-800 border-amber-300 shadow-xs' : 'bg-white hover:bg-slate-100 border-slate-200 text-slate-700'}`}
                    >
                      {density}
                    </button>
                  ))}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white/90 backdrop-blur-xl border border-white/50 rounded-[2rem] p-8 shadow-2xl flex flex-col justify-between hover:shadow-brand/5 transition-all">
              <div>
                ...
                <div className="text-5xl font-black text-slate-900 mb-2 whitespace-nowrap">
                  {material === 'pet' ? `ПЭТ ${selectedThickness}` : `Бумага ${paperDensity}`} <span className="text-2xl font-normal text-text-muted">{material === 'pet' ? 'мкр' : 'г/м²'}</span>
                </div>
              </div>
              <div className="space-y-3 pt-6 border-t border-slate-50">
                <div className="flex justify-between items-center"><span className="text-sm text-text-muted">Вес 1м²:</span><span className="text-lg font-bold font-mono">{displayRU(record.weightPerM2, material === 'pet' ? 4 : 2)} гр</span></div>
      ...
      <footer className="h-10 bg-white/80 backdrop-blur-md border-t border-border-subtle flex items-center justify-between px-6 shrink-0 text-[11px] text-text-muted uppercase tracking-widest font-semibold z-10 sticky bottom-0">
        ...
        <div>Инструментарий пересчёта ПЭТ и бумаги</div>
      </footer>
    </div>
  );
}
